...
const { SECRET_PASS = '12121adsad', TIME_TO_LIVE_TOKEN_MS = 3600000 * 24 * 7 } =
  process.env;
  ...
  User.findUserByCredentials(login, password)
    .then((user) => {
      const token = jwt.sign(
        ...
        { expiresIn: TIME_TO_LIVE_TOKEN_MS/1000 } // надо ли ограничение по времени??!
      );
      ...
      return res
        .cookie('jwt', token, {
          maxAge: TIME_TO_LIVE_TOKEN_MS, // надо ли ограничение по времени??!
          httpOnly: true,
          sameSite: true,
        })
        .send({
